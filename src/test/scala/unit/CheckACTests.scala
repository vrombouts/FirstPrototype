...
class CheckACTests extends UnitSpec {
  ...
  "Calling CheckAC for checking a constraint returning always false with a checker returning false and a filtering process that does nothing" should "have at least one error" in{
    Constraint.checkAC(dummyConstraint,falseChecker)
    assert(Constraint.stats.nbFailedTests > 0)
  }

  "Calling CheckAC for checking a constraint returning always true with a checker returning true and a filtering that always throws an error" should "have at least one error" in{
    Constraint.checkAC(throwExceptionConstraint,trueChecker)
    assert(Constraint.stats.nbFailedTests > 0)
  }

  "Calling CheckAC for checking a constraint returning always true with a checker returning true and a filtering that always returns empty domains" should "have at least one error" in{
    Constraint.checkAC(noSolutionConstraint,trueChecker)
    assert(Constraint.stats.nbFailedTests > 0)
  }

  "Calling CheckAC for checking an constraint that does nothing with a correct checker (returning always true) and a filtering that always returns empty domains" should "have no error" in{
  ...
  "Calling CheckAC Incremental on a constraint that returns always true with an init removing some values " should " have at least a failed test" in{
    ...
    def dummyBranchingFiltering(b:BranchOp) : Array[Set[Int]]={
      b match{
        ...
        case _:Pop => {currentVars = stack.pop(); currentVars}
  ...
  "Calling CheckAC Incremental on a constraint that returns always true with an init removing no value " should " have at least a failed test" in{
    Constraint.gen.reset()
    var currentVars : Array[Set[Int]] = Array()
    var stack:util.Stack[Array[Set[Int]]] = new util.Stack[Array[Set[Int]]]()
    def dummy(x: Array[Set[Int]]):Array[Set[Int]] = {
